...
import static org.apache.lucene.tests.util.LuceneTestCase.assertThat;
import static org.hamcrest.Matchers.anyOf;
import static org.hamcrest.Matchers.equalTo;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertTrue;
import static org.opensearch.neuralsearch.BaseNeuralSearchIT.bulkIngest;
import static org.opensearch.neuralsearch.BaseNeuralSearchIT.makeRequest;
import static org.opensearch.neuralsearch.BaseNeuralSearchIT.toHttpEntity;
...
public class SparseTestCommon {
    private static final String ALGO_NAME = SparseConstants.SEISMIC;
    ...
    private static Request configureSparseIndex(
        SparseEngine engine,
        String indexName,
        String fieldName,
        int nPostings,
        ...
        String indexMappings = prepareIndexMapping(engine, nPostings, alpha, clusterRatio, approximateThreshold, fieldName);
        Request request = new Request("PUT", "/" + indexName);
    ...
    public static String prepareIndexSettings(int shards, int replicas) throws IOException {
        XContentBuilder settingBuilder = XContentFactory.jsonBuilder()
            .startObject()
            .startObject("index")
            .field("number_of_routing_shards", shards)
            .field("sparse", true)
    ...
    /**
     * Writes a {@code sparse_vector} property configured with the seismic method for the given
     * engine. Quantization ceilings are omitted when null, leaving the mapping defaults in place.
     */
    private static void addSeismicField(
        ...
        if (quantizationCeilingIngest != null) {
            mappingBuilder.field("quantization_ceiling_ingest", quantizationCeilingIngest);
        }
        if (quantizationCeilingSearch != null) {
            mappingBuilder.field("quantization_ceiling_search", quantizationCeilingSearch);
        }
        mappingBuilder.endObject().endObject().endObject();
    }

    @SneakyThrows
    public static List<Map<String, Float>> prepareIngestDocuments(int docCount) {
    ...
    @SneakyThrows
    public static void prepareSparseIndex(RestClient client, SparseEngine engine, String index, String sparseField, String textField) {
        int docCount = 100;
        createSparseIndex(client, engine, index, sparseField, 100, 0.4f, 0.1f, docCount);
        List<Map<String, Float>> docs = prepareIngestDocuments(docCount);
        ingestDocumentsAndForceMergeForSingleShard(client, index, textField, sparseField, docs);
    }

    public static void forceMerge(RestClient client, String indexName) throws IOException, ParseException {
    ...
    @SneakyThrows
    public static void prepareMultiShardReplicasIndex(
        RestClient client,
        SparseEngine engine,
        String index,
        String sparseField,
        ...
        int docCount = 100;
        createSparseIndex(client, engine, index, sparseField, 100, 0.4f, 0.1f, docCount, shards, replicas);
        // Verify index exists
        assertTrue(indexExists(client, index));
        // Ingest documents
        List<Map<String, Float>> docs = prepareIngestDocuments(docCount);
        ...
        waitForSegmentMerge(client, index, shards, replicas);
        assertEquals(shards * (replicas + 1), getSegmentCount(client, index));
    }

    @SneakyThrows
    public static void prepareNonSparseIndex(RestClient client, String index) {
        XContentBuilder settingBuilder = XContentFactory.jsonBuilder()
            .startObject()
    ...
    @SneakyThrows
    public static void prepareMixSeismicRankFeaturesIndex(
        RestClient client,
        SparseEngine engine,
        String TEST_INDEX_NAME,
        String TEST_SPARSE_FIELD_NAME,
        String TEST_TEXT_FIELD_NAME
    ) {
        createSparseIndex(client, engine, TEST_INDEX_NAME, TEST_SPARSE_FIELD_NAME, 4, 0.4f, 0.5f, 4);
    ...
    @SneakyThrows
    public static void prepareOnlyRankFeaturesIndex(
        RestClient client,
        SparseEngine engine,
        String TEST_INDEX_NAME,
        String TEST_SPARSE_FIELD_NAME,
    ...
    @SneakyThrows
    public static void createIndexWithMultipleSeismicFields(
        RestClient client,
        SparseEngine engine,
        String indexName,
    ...
    public static void waitForSegmentMerge(RestClient client, String index) throws InterruptedException {
        waitForSegmentMerge(client, index, 1, 0);
    }

    public static void waitForSegmentMerge(RestClient client, String index, int shards, int replicas) throws InterruptedException {
        int maxRetry = 30;
        for (int i = 0; i < maxRetry; ++i) {
            if (shards * (1 + replicas) == getSegmentCount(client, index)) {
                return;
            }
            Thread.sleep(1000);
        }
    ...
    public static int getSegmentCount(RestClient client, String index) {
    ...
    public static int getNodeCount(RestClient client) throws Exception {
    ...
    public static int getDataNodeCount(RestClient client) throws Exception {
        Request request = new Request("GET", "/_cat/nodes/");
        Response response = client.performRequest(request);
        String str = EntityUtils.toString(response.getEntity());
        String[] lines = str.split("\n");
    ...
    @SneakyThrows
    public static void ingestDocumentsAndForceMergeForSingleShard(
        RestClient client,
        String index,
        String textField,
        String sparseField,
        ...
        ingestDocumentsAndForceMergeForSingleShard(client, index, textField, sparseField, docTokens, null);
    }
    ...
    public static String prepareSparseBulkIngestPayload(
        String index,
        String textField,
        String sparseField,
        List<Map<String, Float>> docTokens,
     ...
     */
    public static List<String> generateUniqueRoutingIds(int num) {
    ...
    public static NeuralSparseQueryBuilder getNeuralSparseQueryBuilder(String field, int cut, float hf, int k, Map<String, Float> query) {
        return getNeuralSparseQueryBuilder(field, cut, hf, k, query, null);
    }

    public static NeuralSparseQueryBuilder getNeuralSparseQueryBuilder(
        String field,
        int cut,
        float hf,
        int k,
    ...
    @SneakyThrows
    public static int getEffectiveReplicaCount(RestClient client, int replicas) {
        // effective number of replica is capped by the number of OpenSearch nodes minus 1
        return Math.min(replicas, getDataNodeCount(client) - 1);
    }

    @SuppressWarnings("unchecked")
    public static List<String> getDocIDs(Map<String, Object> searchResults) {
    ...
    protected static String convertTokensToText(Map<String, Float> tokens) {
