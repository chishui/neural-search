...
* [Sparse ANN] Build the native engine index from a memory-mapped CSR file instead of streaming the vectors off-heap, cutting peak anonymous memory during a forcemerge by 42% ([#1802](https://github.com/opensearch-project/neural-search/issues/1802))
* [Sparse ANN] Add a `clustering_batch_size` mapping parameter that bounds the native engine's build memory by clustering the term space in windows ([#1802](https://github.com/opensearch-project/neural-search/issues/1802))
